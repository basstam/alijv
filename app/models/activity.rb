class Activity < ActiveRecord::Base
  ...
  def category_starttime_in_range_start_enddate?
    if   categories.where("start_time is not null").where('(start_time < ?)', self.start_date).count > 0 ||
       ( self.end_date.present? && categories.where("start_time is not null").where('(start_time >= ?)', self.end_date).count > 0)
